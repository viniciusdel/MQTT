...
class MyWindow(QMainWindow):
    ...
    def on_buttonPress(self):
        ...
        publish.single("Vin/ToClient", msg, hostname= "mqtt.eclipse.org")
        self.textEdit.clear()

    @QtCore.pyqtSlot(bool)
    def on_buttonPressWater(self):
        publish.single("Vin/ToClient", "Water please!", hostname= "mqtt.eclipse.org")
    ...
    def on_stateChanged(self, state):
        if state == MqttClient.Connected:
            #print(state)
            self.client.subscribe("Vin/ToServer")
